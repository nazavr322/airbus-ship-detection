...
def _add_upsamling_block(model, previous_layer, encoder_layer):
    """
    Returns a model with an upsampling block added, whose inputs are an
    activations from a previous layer and activations from a corresponding
    layer in encoder.
    """
...
def create_unet50(input_shape: tuple[int], weights: str = None) -> Model:
    """Creates a UNet architecture with pretrained ResNet50 encoder"""
    ...
    model = ResNet50(
        False, resnet_weights, input_tensor=inp, input_shape=input_shape
    )
    ...
    # add 1x1 convolution with and sigmoid to convert embeddings to masks
