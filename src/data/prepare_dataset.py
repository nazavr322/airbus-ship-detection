...
def create_parser() -> ArgumentParser:
    ...
    parser.add_argument('raw_data_path', help='path to .csv file with raw data')
...
# The dataset is built in such a way that each row in the table corresponds to one
# ship mask (e.g imagine picture containing 3 ships => there will be 3 rows with the
# same filename but different rle string in a table).
# Idea of this function is to remove duplicated rows, but keep all the masks.
# It can be achieved by concatenaiting all corresponding masks for each picture.

def remove_duplicates(data: pd.DataFrame) -> pd.DataFrame:
    """
    For each image, concatenates all corresponding masks and deletes it's copies.
    """
